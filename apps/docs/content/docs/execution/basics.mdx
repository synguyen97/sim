...
description: Understanding the fundamental execution flow in Nuggets
---
...
When you run a workflow in Nuggets, the execution engine follows a systematic process to ensure blocks are executed in the correct order with proper data flow.
...
Nuggets offers multiple ways to trigger workflow execution:

### Manual Execution

Run workflows on-demand through the Nuggets interface by clicking the "Run" button. This is perfect for:
